...
namespace UniFramework.Machine
{
	public class StateMachine
	{
		...
		public void AddNode(IStateNode stateNode)
		{
			...
			{
				DebugTool.Error($"State node already existed : {nodeName}");
			}
		}
		...
		public void ChangeState(string nodeName)
		{
			...
			if (node == null)
			{
                DebugTool.Error($"Can not found state node : {nodeName}");
				return;
			}

            DebugTool.Log($"{_curNode.GetType().FullName} --> {node.GetType().FullName}");
			_preNode = _curNode;
			_curNode.OnExit();
			_curNode = node;
			_curNode.OnEnter();
		...
		public System.Object GetBlackboardValue(string key)
		{
			...
			{
                DebugTool.Warning($"Not found blackboard value : {key}");
				return null;
			}
		}
